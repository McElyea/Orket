...
### Fixed
- Capture MessageBuilder prompt inputs before native work while preserving original compaction output sinks.
- Retain required-read metadata, native reads, closure and missing-input log production through cancellation and timeout using existing owners.
- Apply the existing governed PathResolver admission to required preloads without granting reference-root authority.
- Preserve valid prompt output, read ordering, normalization, truncation and card-completion authority.
- `compatibility_status`: `breaking`
- `affected_audience`: `all`
- `migration_requirement`: `required`
- Retain interrupted preparation through native settlement and supply workspace-relative required reads. Migration: `docs/architecture/CONTRACT_DELTA_MESSAGE_READ_OWNERSHIP_D_2026-09-22.md`.
- Wider D/E/CAP, Linux clock, full coverage and whole-lane acceptance remain open.
...
### Fixed
- Own ProductFlow command engines through cleanup and keep witness campaign work on one loop.
- Read replay artifacts without constructing a runtime; retain workers and provider cleanup through interruption.
- Migrate ProductFlow's seat-policy callback and exact-output acceptance to current contracts.
- Bind witness resource observations to checkpoint lease history; refuse missing history without changing verifiers.
- `compatibility_status`: `breaking`
- `affected_audience`: `all`
- `migration_requirement`: `required`
- Async diagnostics callers use an owned native worker; commands use the ProductFlow operation owner.
- Borrowed engines and injected replay callables remain caller-owned. Existing deadlines and report schemas remain.
- Contract: `docs/architecture/CONTRACT_DELTA_SCRIPT_RUNTIME_OWNERSHIP_D_2026-09-22.md`.
...
### Fixed
- Capture selected Kernel run identity and immutable absolute lexical workspaces before native effects.
- Bind each engine's Kernel to its project root; carry admitted roots through queued and nested workers.
- Detach gateway and lifecycle requests before lock or identity-callback waits.
- `compatibility_status`: `breaking`
- `affected_audience`: `all`
- `migration_requirement`: `required`
- Direct implicit start requires an active Kernel owner and native execution; typed run inputs permit pure start.
- Custom runtime input providers implement `create_kernel_run_id`; handles now return absolute workspace paths.
- Bind historical relative handles to their intended roots explicitly; no workspace movement or merging occurs.
- Contract: `docs/architecture/CONTRACT_DELTA_KERNEL_RUN_INPUTS_D_2026-09-22.md`.
...
### Fixed
- Own organization-loop configuration, scans, runtime construction and required cleanup through interruption.
- Restore canonical loop startup and project-root asset discovery; order normalized numeric priority correctly after critical-path weight.
- `compatibility_status`: `compatible`
- `affected_audience`: `all`
- `migration_requirement`: Async organization embeddings await `OrganizationLoop.create()` before `run_forever()`.
- Contract and limits: `docs/architecture/CONTRACT_DELTA_ORGANIZATION_LOOP_OWNERSHIP_D_2026-09-21.md`.
...
### Fixed
- Retain kernel request, response and ledger inputs across asynchronous control-plane publication.
- Prevent caller mutation from changing persisted admission snapshots, commit truth, closeout receipts or operator identity.
- `compatibility_status`: `compatible`
- `affected_audience`: `all`
- `migration_requirement`: `none`
- JSON input boundaries and proof limits: `docs/architecture/CONTRACT_DELTA_KERNEL_PUBLICATION_INPUTS_D_2026-09-20.md`.
...
### Fixed
- Restore configured epic calendar timezones, including default UTC and fixed MST, after the v0.6.52 host-timezone regression.
- Capture the timezone name with the EOS baseline and retain its lookup worker through cancellation before asset reads or card publication.
- `compatibility_status`: `compatible`
- `affected_audience`: `all`
- `migration_requirement`: `none`
- Existing stored card sprints are retained. Correction and proof limits: `docs/architecture/CONTRACT_DELTA_EPIC_CALENDAR_ZONE_D_2026-09-20.md`.

## [0.6.52] - 2026-09-20

### Fixed
- Capture Agent model-family configuration before role reads and refuse malformed operator JSON instead of silently defaulting.
- Use the selected turn clock for Agent journal publication and retain explicit publication timestamps across awaits.
- Capture one epic sprint from the selected runtime clock and immutable baseline before asset reads, preserving existing card values.
- `compatibility_status`: `breaking`
- `affected_audience`: `all`
- `migration_requirement`: `required`
- Use explicit model-family environment decoding and migrate custom epic setup/journal-builder callers. Migration: `docs/architecture/CONTRACT_DELTA_REMAINING_RUNTIME_INPUTS_D_2026-09-20.md`.
...
### Fixed
- Capture one immutable SDK/legacy workload policy for admission, validation, result identity and provenance; environment rotation cannot change admitted limits or redaction.
- Preserve independent policy snapshots for overlapping invocations on the same manager.
- Refuse similarly prefixed material paths and retain native Git clean-state observation through interruption.
