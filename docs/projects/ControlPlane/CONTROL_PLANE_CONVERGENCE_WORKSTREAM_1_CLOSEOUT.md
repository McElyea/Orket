...
62. cards, ODR, and extension workload execution now all resolve through `orket/application/services/control_plane_workload_catalog.py`, with `orket/runtime/workload_adapters.py` reduced to a compatibility shim, extension models reduced to raw manifest data only, `ExtensionManager.run_workload(...)` resolving one canonical extension `WorkloadRecord` before execution and reusing that record in extension provenance instead of minting it later, and a negative governance test now failing if non-allowed modules import or call `build_control_plane_workload_record(...)` or `build_control_plane_workload_record_from_workload_contract(...)`
63. the canonical CLI `--rock` runtime now calls `OrchestrationEngine.run_rock(...)` directly instead of routing through `run_card(...)` type detection, so the named rock entrypoint is explicit while still truthfully remaining a delegated start path in the workload-authority matrix
...
| --- | --- | --- | --- |
| `Workload` | `conflicting` | `conflicting` | Added one canonical projection family plus a shared governed workload catalog covering cards, ODR, extensions, sandbox, top-level cards epic execution, and manual review-run execution. Cards and ODR now route through the shared resolver, extension workload execution now resolves one canonical extension `WorkloadRecord` at workload start and reuses that record in provenance, touched catalog-resolved publishers now carry canonical `WorkloadRecord` objects through run-publication and namespace-mutation helpers instead of restating workload authority as local string-pair aliases, runtime-local adapters are compatibility-only on the touched paths, extension models no longer construct control-plane workload records, and governance now fails if non-allowed modules import or call the low-level builders directly or if those touched publishers reintroduce workload string-pair aliases, but universal start-path authority still does not exist. |
...
1. `CE-01` narrowed, not closed
   Reason: a canonical projection family and shared governed workload catalog now exist, cards start-path builders plus the touched extension-manifest workload projection now route through that shared catalog, touched catalog-resolved publishers now carry canonical `WorkloadRecord` objects through their publication helpers instead of restating workload identity as string-pair aliases, and `orket/runtime/workload_adapters.py` plus extension workload models are compatibility-only on the touched paths, but the `Workload` row remains `conflicting` because start-path authority is not yet universal.
...
This matrix is now machine-enforced by `tests/application/test_control_plane_workload_authority_governance.py`. That governance test fails if a non-test module directly consumes workload authority from `control_plane_workload_catalog.py` without explicit matrix coverage, if touched catalog-resolved publishers reintroduce local `workload_id` / `workload_version` authority aliases, or if `ExecutionPipeline.run_rock(...)` stops delegating while the row below still reads `delegated`.

| Start path | Current authority status | Truthful note |
| --- | --- | --- |
| cards epic execution | `projection-resolved` | `workload.contract.v1` payload plus `WorkloadRecord` projection now resolve through `control_plane_workload_catalog.py`. |
| ODR / run arbiter | `projection-resolved` | `scripts/odr/run_arbiter.py` now emits raw `workload.contract.v1` payload and resolves its `WorkloadRecord` through the canonical seam. |
...
| rock entrypoints that initiate governed execution | `delegated` | the canonical CLI `--rock` path now enters `run_rock(...)` explicitly instead of routing through `run_card(...)` heuristics, but `run_rock` still delegates into sub-epic or sandbox paths instead of minting a standalone rock `WorkloadRecord`; the broader rock start-path story is therefore still part of why the row remains `conflicting`. |

## Surviving projection-only or still-temporary surfaces

Surviving surfaces that remain allowed for now:
1. `orket/runtime/workload_adapters.py` plus extension-manifest workload models under `orket/extensions/`
   Reason: these surfaces now delegate cards start-path workload-contract building and keep extension manifest data parsing separate from canonical control-plane workload authority, so they survive only as compatibility adapters for existing imports and manifest parsing. They are still temporary because broader runtime start paths, rocks, and extension entrypoints do not yet consume one universal governed workload authority surface directly.
...
Remaining gaps:
1. `Workload` still lacks one universal governed start-path authority across every runtime start path; cards, ODR, and extension workload execution now route through the shared resolver, touched catalog-resolved publishers now carry canonical `WorkloadRecord` objects instead of local workload string-pair aliases, and the canonical CLI `--rock` runtime now reaches the delegated rock path explicitly instead of via `run_card(...)` type detection, but broader runtime entrypoints and the rock start-path story still are not fully closed.
