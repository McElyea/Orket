...
12. Slice J matrix runner hardening implemented:
    ...
    - matrix case selection now supports multi-model execution per provider via explicit map input (`--provider-model-map`) and single-provider multi-model expansion, enabling real provider/model grid runs from one command
    - matrix artifact now has a deterministic markdown report renderer (`scripts/companion/render_companion_provider_runtime_report.py`) with stable default output path for recommendation/blocker review
    - matrix output contract is now extracted into durable spec authority (`docs/specs/COMPANION_PROVIDER_RUNTIME_MATRIX_CONTRACT.md`) with dedicated contract tests for complete and partial payload semantics (`tests/scripts/test_companion_provider_runtime_matrix_contract.py`)
    - machine-checkable schema authority is now available (`docs/specs/companion-provider-runtime-matrix.schema.json`) with validator CLI support (`python -m scripts.companion.validate_companion_provider_runtime_matrix`) and contract validation tests (`tests/scripts/test_validate_companion_provider_runtime_matrix.py`)
