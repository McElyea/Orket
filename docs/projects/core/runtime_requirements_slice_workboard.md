...
| CORE-IMP-02 | Golden Replay Integrity | in_progress | P0 | CORE-IMP-00, CORE-IMP-01 | Integration replay tests + compatibility rejection tests | `replay_integrity_test_report.json` | `tests/reports/replay_integrity_test_report.json` | Replay now emits `runtime_contract_hash` from runtime contract snapshots + `runtime_policy_versions`, fails closed on incompatible/mixed `ledger_schema_version`, enforces replay-mode operation-record-only execution (`E_REPLAY_OPERATION_MISSING` on gaps), adds strict replay compatibility/completeness rejections (`E_REPLAY_COMPATIBILITY_MISMATCH`, `E_REPLAY_ARTIFACTS_MISSING`, `E_REPLAY_INCOMPLETE`), and emits deterministic `drift_report` classification (`drift_schema_version` `1.0`) across replay compare/campaign outputs; remaining work is capability/workspace compatibility enforcement plus side-effect isolation guardrails. |
