...
| CORE-IMP-02 | Golden Replay Integrity | in_progress | P0 | CORE-IMP-00, CORE-IMP-01 | Integration replay tests + compatibility rejection tests | `replay_integrity_test_report.json` | `tests/reports/replay_integrity_test_report.json` | Replay now emits `runtime_contract_hash` from runtime contract snapshots and fails closed on incompatible/mixed `ledger_schema_version`; remaining replay-isolation, compatibility matrix rejection, and drift-classifier proof obligations are still pending. |
