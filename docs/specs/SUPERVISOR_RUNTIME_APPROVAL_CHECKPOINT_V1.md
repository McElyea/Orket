...
It selects two bounded governed approval lifecycles that have now shipped.
...
3. one pending / inspect / approve-or-deny / explicit continue-or-stop lifecycle per admitted slice
4. one cold continuation rule for the kernel slice: no resume by implication; any surfaced checkpoint remains `resume_forbidden`
5. one bounded same-attempt pre-effect continuation rule for the governed turn-tool `write_file` slice only
6. one attribution and evidence story rooted in durable control-plane records

Out of scope:
1. governed turn-tool approve-to-continue beyond `write_file`
2. broader namespace scopes
3. same-attempt or replacement-attempt resume authority derived from checkpoint presence outside the selected `write_file` slice
...
5. the only admitted turn-tool approval-required continuation slice is `write_file`
...
6. requester: runtime-owned governed turn-tool dispatch on the default `issue:<issue_id>` path only
7. approver or rejector: an authenticated operator on `POST /v1/approvals/{approval_id}/decision`
8. runtime truth owner: durable control-plane records for the selected governed target run, attempt, step, effect-journal, and final-truth lineage when present
...
2. runtime creates a pending approval hold when governed turn-tool dispatch encounters the admitted `write_file` slice before execution
...
7. `approve` on the admitted turn-tool `write_file` slice allows one runtime-owned same-governed-run continue step by replaying the accepted pre-effect checkpoint on the already-selected `control_plane_target_ref`
8. `deny` on the admitted turn-tool `write_file` slice terminal-stops that same governed turn-tool run without hidden continuation
9. Packet 1 does not define a separate manual resume API for either selected slice
10. checkpoint artifacts, when surfaced on these slices, remain inspection and attribution surfaces except for the admitted runtime-owned `write_file` same-attempt continuation rule below
...
3. the admitted turn-tool `write_file` slice may consume the already-accepted same-attempt pre-effect checkpoint only through runtime-owned continuation on the already-selected governed run
...
6. issue namespace authority required by the admitted turn-tool `write_file` target run
7. admitted `write_file` request identity alignment between approved request payload, selected target run, and resumed checkpoint snapshot
...
5. governed turn-tool `write_file` dispatch on the default `issue:<issue_id>` path using `request_type=tool_approval`, `reason=approval_required_tool:write_file`, and the existing `control_plane_target_ref`
...
5. `python -m pytest -q tests/application/test_turn_executor_middleware.py -k "write_file_approval_resume_continues_same_governed_run"`
6. `python -m pytest -q tests/application/test_engine_approvals.py -k "continues_write_file_slice_on_same_session_issue or denial_closes_open_write_file_governed_run or namespace_drift"`
