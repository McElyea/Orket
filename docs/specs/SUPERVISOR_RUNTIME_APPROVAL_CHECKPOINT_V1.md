...
3. one governed turn-tool `create_directory` approval-required slice on the default `issue:<issue_id>` namespace scope
4. one governed turn-tool `create_issue` approval-required slice on the default `issue:<issue_id>` namespace scope
...
3. governed turn-tool `create_directory` approval-required admission on `issue:<issue_id>`
4. governed turn-tool `create_issue` approval-required admission on `issue:<issue_id>`
5. one pending / inspect / approve-or-deny / explicit continue-or-stop lifecycle per admitted slice
6. one cold continuation rule for the kernel slice: no resume by implication; any surfaced checkpoint remains `resume_forbidden`
7. one bounded same-attempt pre-effect continuation rule for the governed turn-tool `write_file`, `create_directory`, and `create_issue` slices only
8. one attribution and evidence story rooted in durable control-plane records

Out of scope:
1. governed turn-tool approve-to-continue beyond `write_file`, `create_directory`, and `create_issue`
2. broader namespace scopes
3. same-attempt or replacement-attempt resume authority derived from checkpoint presence outside the selected `write_file`, `create_directory`, plus `create_issue` slices
...
5. the only admitted turn-tool approval-required continuation slices are `write_file`, `create_directory`, and `create_issue`
...
6. gated action: governed turn-tool `create_directory` request with `request_type=tool_approval` and `reason=approval_required_tool:create_directory`
7. gated action: governed turn-tool `create_issue` request with `request_type=tool_approval` and `reason=approval_required_tool:create_issue`
8. requester: runtime-owned governed turn-tool dispatch on the default `issue:<issue_id>` path only
9. approver or rejector: an authenticated operator on `POST /v1/approvals/{approval_id}/decision`
10. runtime truth owner: durable control-plane records for the selected governed target run, attempt, step, effect-journal, and final-truth lineage when present
...
2. runtime creates a pending approval hold when governed turn-tool dispatch encounters the admitted `write_file`, `create_directory`, or `create_issue` slice before execution
...
7. `approve` on the admitted turn-tool `write_file`, `create_directory`, or `create_issue` slice allows one runtime-owned same-governed-run continue step by replaying the accepted pre-effect checkpoint on the already-selected `control_plane_target_ref`
8. `deny` on the admitted turn-tool `write_file`, `create_directory`, or `create_issue` slice terminal-stops that same governed turn-tool run without hidden continuation
9. Packet 1 does not define a separate manual resume API for either selected slice
10. checkpoint artifacts, when surfaced on these slices, remain inspection and attribution surfaces except for the admitted runtime-owned `write_file`, `create_directory`, plus `create_issue` same-attempt continuation rule below
...
3. the admitted turn-tool `write_file`, `create_directory`, and `create_issue` slices may consume the already-accepted same-attempt pre-effect checkpoint only through runtime-owned continuation on the already-selected governed run
...
6. issue namespace authority required by the admitted turn-tool `write_file`, `create_directory`, or `create_issue` target run
7. admitted `write_file`, `create_directory`, or `create_issue` request identity alignment between approved request payload, selected target run, and resumed checkpoint snapshot
...
5. governed turn-tool `write_file`, `create_directory`, and `create_issue` dispatch on the default `issue:<issue_id>` path using `request_type=tool_approval`, `reason=approval_required_tool:<tool_name>`, and the existing `control_plane_target_ref`
...
8. `ORKET_DISABLE_SANDBOX=1 python -m pytest -q tests/adapters/test_filesystem_create_directory.py tests/application/test_turn_tool_dispatcher_support.py`
