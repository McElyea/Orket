...
| resource | `required_authority` | `authority_lineage.resource` | Does the latest retained resource observation for the checkpoint's dependent lease agree with that lease authority? | Missing or contradictory resource authority fails as `resource_or_lease_evidence_missing` or `resource_lease_consistency_not_verified`. |
