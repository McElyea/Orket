...
4. llama.cpp is the default for every provider-neutral runtime and tool. Shared defaults live in `orket/runtime/config/defaults.py`. LM Studio and Ollama require explicit selection; an unavailable llama.cpp server must never trigger a provider switch. Ollama installation or live coverage is not a prerequisite for llama.cpp work. Existing admission gates still apply.
