...
1. Dependency layering exceptions:
   1. The 0.6.37 generated graph has no forbidden static pairs or cross-layer cycles, but six unresolved dynamic-import/reflection sites still fail its verdict. Core purity and runtime ownership remain separate C/D obligations; zero static pairs does not establish full conformance.
