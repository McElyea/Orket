...
## Active Execution
1. techdebt -- Phase 1 complete (semantic containment, symlink refusal, streaming SHA-256, size caps, deterministic ordered failure payloads). Phase 2 in progress: immutable commit pinning/detached checkout, manifest digest persistence+verification, source trust policy mode/profile checks, compatibility fallback metadata/provenance snapshot, compatibility fallback expiry registry/check script, and compatibility warning artifact exporter implemented; continue full host/protocol matrix coverage and enforcement-flip gates.
