...
Dry-run first:
```bash
python scripts/gitea/publish_products_to_gitea.py
```

Execute publish:
```bash
python scripts/gitea/publish_products_to_gitea.py --execute --verify-parity --private --force
```

Flexible source directory:
```bash
python scripts/gitea/publish_products_to_gitea.py --execute --verify-parity --source-dir bin/projects
```

Optional local deletion after verification:
```bash
python scripts/gitea/publish_products_to_gitea.py --execute --verify-parity --delete-local --source-dir product
```

Publish selected projects only:
```bash
python scripts/gitea/publish_products_to_gitea.py --execute --projects sneaky_price_watch price_arbitrage
```
