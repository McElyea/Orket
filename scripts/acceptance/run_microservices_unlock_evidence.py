...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--matrix-out",
        default="benchmarks/results/acceptance/monolith_variant_matrix.json",
    )
    parser.add_argument(
        "--live-report-out",
        default="benchmarks/results/acceptance/live_acceptance_patterns.json",
    )
    parser.add_argument(
        "--unlock-out",
...
def build_command_plan(args: argparse.Namespace) -> List[List[str]]:
    matrix_cmd = [
        "python",
        "scripts/acceptance/run_monolith_variant_matrix.py",
        "--execute",
        "--models",
        *list(args.models),
        "--iterations",
    ...
    live_loop_cmd = [
        "python",
        "-m",
        "scripts.acceptance.run_live_acceptance_loop",
        "--models",
        *list(args.models),
        "--iterations",
        str(args.iterations),
    ...
    unlock_cmd = [
        "python",
        "scripts/acceptance/check_microservices_unlock.py",
        "--matrix",
        str(args.matrix_out),
        "--readiness-policy",
    ...
    decision_cmd = [
        "python",
        "scripts/acceptance/decide_microservices_pilot.py",
        "--unlock-report",
        str(args.unlock_out),
        "--out",
