...
def _run_checked_scenario(path: Path, timeout_s: float, *, gate_run_id: str, loop_index: int) -> bool:
    ...
    module_name = "scripts.streaming.run_provider_scenario_direct" if direct_provider_scenario else "scripts.streaming.run_stream_scenario"
    cmd = [
        sys.executable,
        "-m",
        module_name,
...
def main() -> int:
    ...
    parser.add_argument(
        "--model-id",
        default=None,
        help="Optional real-provider model id override (otherwise env/default/preflight auto-selection is used).",
    )
    parser.add_argument(
        "--auto-select-model",
        action="store_true",
        help="Allow preflight to auto-select a provider-compatible model when requested/default model is unavailable.",
    )
    parser.add_argument(
        "--stability-loops",
        type=int,
    ...
    if args.provider_mode == "real" and str(args.model_id or "").strip():
        os.environ["ORKET_MODEL_STREAM_REAL_MODEL_ID"] = str(args.model_id).strip()
    ...
    if args.provider_mode == "real" and not args.skip_preflight:
        cmd = [sys.executable, str(PROJECT_ROOT / "scripts" / "providers" / "check_model_provider_preflight.py")]
        if args.real_provider:
            cmd.extend(["--provider", args.real_provider])
        if str(args.model_id or "").strip():
            cmd.extend(["--model-id", str(args.model_id).strip()])
        if bool(args.auto_select_model):
            cmd.append("--auto-select-model")
        if args.preflight_smoke_stream:
            cmd.append("--smoke-stream")
        preflight = subprocess.run(cmd, cwd=PROJECT_ROOT, capture_output=True, text=True, check=False)
        if preflight.stdout:
        ...
        resolved_model = _extract_line_value(preflight.stdout or "", "RESOLVED_MODEL_ID")
        if resolved_model:
            os.environ["ORKET_MODEL_STREAM_REAL_MODEL_ID"] = resolved_model
