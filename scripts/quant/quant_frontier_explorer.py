...
try:
    from scripts.common.rerun_diff_ledger import append_payload_history
except ModuleNotFoundError:  # pragma: no cover - direct script execution fallback
    import sys

    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from common.rerun_diff_ledger import append_payload_history
...
def _append_store(payload: dict[str, Any], storage_root: Path) -> Path:
    ...
    append_payload_history(store_path, payload, history_key="history")
    return store_path
