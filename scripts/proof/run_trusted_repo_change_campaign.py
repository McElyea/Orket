...
from scripts.proof.trusted_repo_change_contract import DEFAULT_CAMPAIGN_OUTPUT, DEFAULT_WORKSPACE_ROOT, PROOF_RESULTS_ROOT, relative_to_repo
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the First Useful Workflow Slice verification campaign.")
    parser.add_argument("--workspace-root", default=str(DEFAULT_WORKSPACE_ROOT), help="Fixture workspace root.")
    parser.add_argument("--runs", type=int, default=2, help="Number of equivalent approved executions.")
...
def _run_campaign(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for index in range(1, max(1, int(args.runs)) + 1):
        live = execute_trusted_repo_change(workspace_root=workspace, scenario="approved", run_index=index)
        live_path = PROOF_RESULTS_ROOT / f"trusted_repo_change_live_run_{index:02d}.json"
    ...
    return build_trusted_repo_change_campaign_report(reports, bundle_refs=bundle_refs, live_proof_refs=live_refs)
