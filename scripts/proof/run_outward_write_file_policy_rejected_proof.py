...
from scripts.proof.outward_run_witness_contract import COMPARE_SCOPE_POLICY_REJECTED
...
PACKAGE_OUTPUT = PROOF_ROOT / "outward_run_policy_rejected_witness_package.v1"
VERIFIER_OUTPUT = PROOF_ROOT / "outward_run_policy_rejected_witness_report.json"
                ...
                "tool_calls": [
                    {
                        ...
                        "function": {
                            "name": "write_file",
                            "arguments": {"path": "../policy-rejected-proof-output.txt", "content": "blocked proof content"},
                        },
                    }
                ],
                "provider_name": "proof-local",
                "provider_backend": "proof-local",
                "model": "deterministic-policy-rejection-proof-client",
                "usage": {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2},
                "latency_ms": 0,
                "orket_session_id": "proof-policy-rejection-session",
...
async def run_proof(*, proof_root: Path = PROOF_ROOT, package_output: Path = PACKAGE_OUTPUT) -> dict[str, Any]:
    ...
    live_root = Path(".tmp/outward-write-file-policy-rejected-proof").resolve()
    if live_root.exists():
    ...
    run_id = "run-outward-write-file-policy-rejected-proof"
    clock = _Clock()
    ...
    run_service = OutwardRunService(
        run_store=run_store,
        event_store=event_store,
        run_id_factory=lambda: "outward-policy-rejection-proof",
        utc_now=clock,
    )
    run = await run_service.submit(_submission(run_id))
    completed = await execution_service.start_if_ready(run.run_id)
    events = await event_store.list_for_run(run.run_id)
    event_types = [event.event_type for event in events]
    target = live_root / "policy-rejected-proof-output.txt"
    if completed.status != "completed" or completed.stop_reason is None:
        return _blocked("run_not_policy_rejected_completed", run=completed.to_status_payload())
    if target.exists():
        return _blocked("policy_rejected_effect_was_written", run=completed.to_status_payload())
    if await approval_store.list(status=None, run_id=run.run_id):
        return _blocked("policy_rejected_approval_was_created", run=completed.to_status_payload())
    if "proposal_policy_rejected" not in event_types:
        return _blocked("policy_rejection_event_missing", run=completed.to_status_payload())
    if "tool_invoked" in event_types or "commitment_recorded" in event_types:
        return _blocked("policy_rejected_effect_event_present", run=completed.to_status_payload())
    ...
    verifier = verify_package(package_output, scope=COMPARE_SCOPE_POLICY_REJECTED)
    corruption = run_corruption_suite(policy_rejected_base=package_output)
    write_payload_with_diff_ledger(VERIFIER_OUTPUT, verifier)
    write_payload_with_diff_ledger(CORRUPTION_OUTPUT, corruption)
    success = verifier.get("result") == "accepted" and corruption.get("result") == "accepted"
    return {
        "schema_version": "outward_write_file_policy_rejected_proof_run.v1",
...
def _submission(run_id: str) -> dict[str, Any]:
    return {
        ...
        "task": {
            "description": "Attempt a policy-rejected outward proof file.",
            "instruction": "Produce exactly one write_file tool call outside the workspace.",
            "acceptance_contract": {
                "governed_tool_call": {
                    "tool": "write_file",
                    "args": {"path": "policy-contract-placeholder.txt", "content": "contract placeholder"},
                }
            },
        },
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the outward write_file policy-rejected proof chain.")
    parser.add_argument("--package-output", default=str(PACKAGE_OUTPUT), help="Policy-rejected witness package output directory.")
    parser.add_argument("--output", default=str(PROOF_ROOT / "outward_write_file_policy_rejected_proof_run.json"))
