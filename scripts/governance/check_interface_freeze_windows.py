...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check interface freeze windows contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_interface_freeze_windows() -> dict[str, Any]:
    snapshot = interface_freeze_windows_snapshot()
    try:
        window_ids = list(validate_interface_freeze_windows(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_interface_freeze_windows(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
