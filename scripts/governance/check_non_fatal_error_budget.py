...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check non-fatal error budget contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_non_fatal_error_budget() -> dict[str, Any]:
    snapshot = non_fatal_error_budget_snapshot()
    try:
        budget_ids = list(validate_non_fatal_error_budget(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
            "budget_count": len(budget_ids),
...
def check_non_fatal_error_budget(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
