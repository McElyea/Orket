...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check idempotency discipline policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_idempotency_discipline_policy() -> dict[str, Any]:
    snapshot = idempotency_discipline_policy_snapshot()
    try:
        surfaces = list(validate_idempotency_discipline_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_idempotency_discipline_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
