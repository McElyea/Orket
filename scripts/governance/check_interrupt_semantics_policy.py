...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check interrupt semantics policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_interrupt_semantics_policy() -> dict[str, Any]:
    snapshot = interrupt_semantics_policy_snapshot()
    try:
        surfaces = list(validate_interrupt_semantics_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_interrupt_semantics_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
    payload = evaluate_interrupt_semantics_policy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, payload = check_interrupt_semantics_policy(out_path=out_path)
