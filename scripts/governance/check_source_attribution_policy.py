...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check source attribution policy contract.")
    parser.add_argument("--out", default="", help="Optional output JSON path.")
    return parser.parse_args(argv)


def evaluate_source_attribution_policy() -> dict[str, Any]:
    snapshot = source_attribution_policy_snapshot()
    try:
        modes = list(validate_source_attribution_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_source_attribution_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, payload = check_source_attribution_policy(out_path=out_path)
