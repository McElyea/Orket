...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check evidence package generator contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_evidence_package_generator_contract() -> dict[str, Any]:
    snapshot = evidence_package_generator_contract_snapshot()
    try:
        required_sections = list(validate_evidence_package_generator_contract(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_evidence_package_generator_contract(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
    payload = evaluate_evidence_package_generator_contract()
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, payload = check_evidence_package_generator_contract(out_path=out_path)
