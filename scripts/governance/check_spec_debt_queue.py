...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check spec debt queue contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_spec_debt_queue() -> dict[str, Any]:
    snapshot = spec_debt_queue_snapshot()
    try:
        debt_ids = list(validate_spec_debt_queue(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
            "debt_count": len(debt_ids),
            "debt_ids": debt_ids,
            "snapshot": snapshot,
        }
...
def check_spec_debt_queue(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
