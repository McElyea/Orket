...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check clock/time authority policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_clock_time_authority_policy() -> dict[str, Any]:
    snapshot = clock_time_authority_policy_snapshot()
    try:
        policy = validate_clock_time_authority_policy(snapshot)
    ...
    return {
        ...
        "ok": True,
        "defaults": defaults,
        "source_precedence": list(policy.get("source_precedence") or []),
        "snapshot": snapshot,
    }


def check_clock_time_authority_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
