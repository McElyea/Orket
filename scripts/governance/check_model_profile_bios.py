...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check model profile BIOS contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_model_profile_bios() -> dict[str, Any]:
    snapshot = model_profile_bios_snapshot()
    try:
        profile_ids = list(validate_model_profile_bios(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_model_profile_bios(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
