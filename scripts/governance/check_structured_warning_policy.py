...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check structured warning policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_structured_warning_policy() -> dict[str, Any]:
    snapshot = structured_warning_policy_snapshot()
    try:
        warning_codes = list(validate_structured_warning_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_structured_warning_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
