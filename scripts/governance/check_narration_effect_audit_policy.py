...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check narration-to-effect audit policy contract.")
    parser.add_argument("--out", default="", help="Optional output JSON path.")
    return parser.parse_args(argv)


def evaluate_narration_effect_audit_policy() -> dict[str, Any]:
    snapshot = narration_effect_audit_policy_snapshot()
    try:
        tools = list(validate_narration_effect_audit_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_narration_effect_audit_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, payload = check_narration_effect_audit_policy(out_path=out_path)
