...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check execution-readiness rubric contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_execution_readiness_rubric() -> dict[str, Any]:
    snapshot = execution_readiness_rubric_snapshot()
    try:
        criteria = list(validate_execution_readiness_rubric(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_execution_readiness_rubric(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
    payload = evaluate_execution_readiness_rubric()
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, payload = check_execution_readiness_rubric(out_path=out_path)
