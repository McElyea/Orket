...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check failure replay harness contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_failure_replay_harness_contract() -> dict[str, Any]:
    snapshot = failure_replay_harness_contract_snapshot()
    try:
        required_output_fields = list(validate_failure_replay_harness_contract(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_failure_replay_harness_contract(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
