...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check operator override logging policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_operator_override_logging_policy() -> dict[str, Any]:
    snapshot = operator_override_logging_policy_snapshot()
    try:
        override_types = list(validate_operator_override_logging_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
            "override_type_count": len(override_types),
...
def check_operator_override_logging_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
