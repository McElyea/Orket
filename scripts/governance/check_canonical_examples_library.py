...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check canonical examples library contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_canonical_examples_library() -> dict[str, Any]:
    snapshot = canonical_examples_library_snapshot()
    try:
        example_ids = list(validate_canonical_examples_library(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_canonical_examples_library(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
