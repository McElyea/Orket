...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check workspace hygiene rules contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_workspace_hygiene_rules() -> dict[str, Any]:
    snapshot = workspace_hygiene_rules_snapshot()
    try:
        rule_ids = list(validate_workspace_hygiene_rules(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_workspace_hygiene_rules(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
