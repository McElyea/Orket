...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check sampling discipline guide contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_sampling_discipline_guide() -> dict[str, Any]:
    snapshot = sampling_discipline_guide_snapshot()
    try:
        event_classes = list(validate_sampling_discipline_guide(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_sampling_discipline_guide(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
