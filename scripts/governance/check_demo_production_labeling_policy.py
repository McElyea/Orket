...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check demo-vs-production labeling policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_demo_production_labeling_policy() -> dict[str, Any]:
    snapshot = demo_production_labeling_policy_snapshot()
    try:
        labels = list(validate_demo_production_labeling_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_demo_production_labeling_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
