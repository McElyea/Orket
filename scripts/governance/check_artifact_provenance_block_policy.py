...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check strict artifact provenance block policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_artifact_provenance_block_policy() -> dict[str, Any]:
    snapshot = artifact_provenance_block_policy_snapshot()
    try:
        required_fields = list(validate_artifact_provenance_block_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_artifact_provenance_block_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
    payload = evaluate_artifact_provenance_block_policy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, payload = check_artifact_provenance_block_policy(out_path=out_path)
