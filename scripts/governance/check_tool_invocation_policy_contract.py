...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check tool-invocation policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_tool_invocation_policy_contract() -> dict[str, Any]:
    snapshot = tool_invocation_policy_contract_snapshot()
    try:
        run_types = list(validate_tool_invocation_policy_contract(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
            "run_type_count": len(run_types),
            "run_types": run_types,
            "snapshot": snapshot,
        }
...
def check_tool_invocation_policy_contract(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
