...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check local-vs-remote route policy contract.")
    parser.add_argument(
        "--out",
        default="",
...
def evaluate_local_remote_route_policy() -> dict[str, Any]:
    snapshot = local_remote_route_policy_snapshot()
    try:
        lanes = list(validate_local_remote_route_policy(snapshot))
        return {
            "schema_version": "1.0",
            "ok": True,
...
def check_local_remote_route_policy(*, out_path: Path | None = None) -> tuple[int, dict[str, Any]]:
