...
def _render_readme(index: dict[str, Any]) -> str:
    ...
    lines.append("```bash")
    lines.append("python scripts/MidTier/sync_published_index.py --write")
    lines.append("```")
    lines.append("4. Validate before commit:")
    lines.append("```bash")
    lines.append("python scripts/MidTier/sync_published_index.py --check")
    lines.append("```")
...
def main() -> int:
    ...
    if args.check:
        ...
        if existing != rendered:
            raise SystemExit("E_README_OUT_OF_SYNC run: python scripts/MidTier/sync_published_index.py --write")
