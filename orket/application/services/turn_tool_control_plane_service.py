...
from orket.application.services.turn_tool_control_plane_closeout import (
    ...
    finalize_turn_execution_atomic,
...
from orket.application.services.turn_tool_recovery_transaction import turn_tool_transaction
from orket.application.services.turn_tool_step_publication import prepare_tool_dispatch, publish_step_result_atomic
...
from orket.core.contracts.local_file_lock import LocalFileLocks
...
class TurnToolControlPlaneService:
    ...
    def __init__(
        ...
        *,
        ...
        execution_owners: LocalFileLocks,
    ) -> None:
        self.execution_repository = execution_repository
        self.publication = publication
        self.transactions = transactions
        self.execution_owners = execution_owners

    def _borrow_transaction(self, transaction: ControlPlaneTransaction) -> TurnToolControlPlaneService:
        return TurnToolControlPlaneService(
            ...
            transactions=self.transactions, execution_owners=self.execution_owners,
        )

    async def publish_preflight_failure(
        self, *, session_id: str, issue_id: str, role_name: str, turn_index: int,
        proposal_hash: str, violation_reasons: list[str],
    ) -> tuple[RunRecord, FinalTruthRecord]:
        async with self.transactions() as transaction:
            scoped = self._borrow_transaction(transaction)
            return await scoped._publish_preflight_failure(
                session_id=session_id, issue_id=issue_id, role_name=role_name, turn_index=turn_index,
                proposal_hash=proposal_hash, violation_reasons=violation_reasons,
            )

    async def _publish_preflight_failure(
        self, *, session_id: str, issue_id: str, role_name: str, turn_index: int,
        proposal_hash: str, violation_reasons: list[str],
    ) -> tuple[RunRecord, FinalTruthRecord]:
        ...
        if current_attempt is not None and not is_terminal_attempt_state(current_attempt.attempt_state):
            ...
            if current_attempt.recovery_decision_id is None:
                await self.publication.publish_recovery_decision(
            ...
            current_attempt = AttemptRecord.model_validate({
                **current_attempt.model_dump(), "attempt_state": AttemptState.ABANDONED,
                "end_timestamp": closed_at, "side_effect_boundary_class": None,
                "failure_class": None, "failure_plane": None, "failure_classification": None,
                "recovery_decision_id": None,
            })
            current_attempt = await self.execution_repository.save_attempt_record(record=current_attempt)
    ...
    async def begin_execution(
        ...
        *,
        ...
        async with turn_tool_transaction(self.transactions) as transaction:
            scoped = self._borrow_transaction(transaction)
            return await scoped._begin_execution(session_id=session_id, issue_id=issue_id, role_name=role_name,
                turn_index=turn_index, proposal_hash=proposal_hash, resume_mode=resume_mode)

    async def _begin_execution(
        self, *, session_id: str, issue_id: str, role_name: str, turn_index: int,
        proposal_hash: str, resume_mode: bool,
    ) -> tuple[RunRecord, AttemptRecord]:
        run = await self._ensure_admission_pending_run(
            session_id=session_id, issue_id=issue_id, role_name=role_name,
            turn_index=turn_index, proposal_hash=proposal_hash,
        )
    ...
    async def publish_step_result(
        ...
        *,
        ...
        return await publish_step_result_atomic(
            transactions=self.transactions, authority=self.publication.authority,
            run_id=run_id, attempt_id=attempt_id, step_id=step_id, tool_name=tool_name,
            tool_args=tool_args, result=result, binding=binding, operation_id=operation_id,
            replayed=replayed, error_type=TurnToolControlPlaneError,
        )

    async def prepare_dispatch(
        self, *, run_id: str, attempt_id: str, step_id: str, tool_name: str,
        tool_args: dict[str, Any], binding: dict[str, Any] | None, operation_id: str,
    ) -> StepRecord:
        return await prepare_tool_dispatch(
            transactions=self.transactions, authority=self.publication.authority,
            run_id=run_id, attempt_id=attempt_id, step_id=step_id, tool_name=tool_name,
    ...
    async def finalize_execution(
        self, *, run_id: str, attempt_id: str, authoritative_result_ref: str,
        violation_reasons: list[str], executed_step_count: int,
        ...
        *,
        ...
        if run is not None:
            if not is_terminal_run_state(run.lifecycle_state):
                await require_turn_dispatch_contract(self.publication.repository, run, TurnToolControlPlaneError)
                await ensure_admission_reservation(publication=self.publication, run=run)
            return run
        policy_payload = {"proposal_hash": proposal_hash, "governed": True}
        configuration_payload = {
            "dispatch_contract": TURN_TOOL_DISPATCH_CONTRACT,
