...
from orket.application.services.review_run_closeout import finalize_review_run, read_review_terminal
...
class ReviewRunControlPlaneService:
    ...
    async def finalize_failed_if_started(
        ...
        *,
        ...
        async with self.transactions() as transaction:
            owner = self._transaction_service(transaction)
            if await transaction.execution.get_run_record(run_id=run_id) is None:
                return None
            return await finalize_review_run(
                owner, run_id=run_id, failed=True, failure_class=failure_class, preserve_terminal=True,
            )

    def _transaction_service(self, transaction: ControlPlaneTransaction) -> ReviewRunControlPlaneService:
    ...
    async def read_execution_summary(self, *, run_id: str) -> dict[str, object]:
        async with self.transactions() as transaction:
            return await self._transaction_service(transaction)._read_execution_summary(run_id=run_id)

    async def _read_execution_summary(self, *, run_id: str) -> dict[str, object]:
        run = await self._require_run(run_id=run_id)
        attempt, _ = await read_review_terminal(self, run)
    ...
    async def _finalize(
        ...
        *,
        ...
        async with self.transactions() as transaction:
            return await finalize_review_run(
                self._transaction_service(transaction), run_id=run_id, failed=failed, failure_class=failure_class,
            )
