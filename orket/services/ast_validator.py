    ...
    def _check_class_rules(node: ast.ClassDef, category: str) -> List[ASTRuleViolation]:
        ...
        if category == "manager" and "manager" not in name:
            violations.append(
                ASTRuleViolation(
                    line=node.lineno,
                    message=f"Class '{node.name}' in a Manager component must end with 'Manager' suffix.",
                )
            )
        if category == "engine" and "engine" not in name:
            violations.append(
                ASTRuleViolation(
                    line=node.lineno,
                    message=f"Class '{node.name}' in an Engine component must end with 'Engine' suffix.",
                )
            )
        if category == "accessor" and "accessor" not in name:
            violations.append(
                ASTRuleViolation(
                    line=node.lineno,
                    message=f"Class '{node.name}' in an Accessor component must end with 'Accessor' suffix.",
                )
            )
            ...
            violations.append(
                ASTRuleViolation(
                    ...
                    message=(
                        ...
                        "iDesign recommends splitting high-complexity components."
                    ),
                    severity="warning",
                )
            )
