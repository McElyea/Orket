...
class GiteaVendor(VendorInterface):
    ...
    async def add_card(self, epic_id: str, summary: str, description: str) -> VendorCard:
        ...
        resp = await self._client.post(
            f"{self.repo_api}/issues",
            json={"title": summary, "body": description, "labels": [label_id] if label_id is not None else []},
        )
        i = resp.json()
    ...
    async def get_card_details(self, card_id: str) -> VendorCard:
        ...
        i = resp.json()
        return VendorCard(
            id=str(i["number"]), summary=i["title"], description=i["body"], status=i["state"], priority="Medium"
        )
