...
class OrchestrationConfig:
    ...
    def __init__(self, org: Organization | None, *, environment: Mapping[str, str] | None = None) -> None:
        self.org = org
    ...
    def resolve_state_backend_mode(self, *, user_settings: dict[str, Any] | None = None) -> str:
        env_raw = (self._environment.get("ORKET_STATE_BACKEND_MODE") or "").strip()
    ...
    def resolve_run_ledger_mode(self, *, user_settings: dict[str, Any] | None = None) -> str:
        env_raw = (self._environment.get("ORKET_RUN_LEDGER_MODE") or "").strip()
    ...
    def resolve_gitea_state_pilot_enabled(self, *, user_settings: dict[str, Any] | None = None) -> bool:
        env_raw = (self._environment.get("ORKET_ENABLE_GITEA_STATE_PILOT") or "").strip()
    ...
    def validate_state_backend_mode(self, state_backend_mode: str, gitea_state_pilot_enabled: bool) -> None:
        if state_backend_mode != "gitea":
            return
        env_mode = (self._environment.get("ORKET_STATE_BACKEND_MODE") or "").strip().lower()
        env_pilot_raw = (self._environment.get("ORKET_ENABLE_GITEA_STATE_PILOT") or "").strip()
        ...
        readiness = evaluate_gitea_state_pilot_readiness(collect_gitea_state_pilot_inputs(environment=self._environment))
