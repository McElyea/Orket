...
class StateMachine:
    ...
    @staticmethod
    def validate_transition(card_type: CardType, current: CardStatus, requested: CardStatus, role: str = "system"):
        """
        Validates if a role is allowed to perform a specific state change.
        ...
        # 2. Role-Based Enforcement (The 'Integrity' Gate)
        if card_type == CardType.ISSUE:
            # Only the 'integrity_guard' (Verifier) can move an issue to DONE
            if requested == CardStatus.DONE and role != "integrity_guard":
                raise StateMachineError(f"Permission Denied: Only the 'integrity_guard' role can finalize Issues to 'DONE'. Current role: {role}")
