...
28. Canonical external extension host-validation path: `orket ext validate <extension_root> --strict --json`; the active SupervisorRuntime Packet 1 validation seam admits only `manifest_version: v0` and must fail closed on any unsupported manifest family before extension execution authority is considered
