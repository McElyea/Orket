...
def test_runtime_truth_contract_drift_report_passes_for_current_contracts() -> None:
    ...
    assert "narration_effect_audit_policy_valid" in checks
    assert "source_attribution_policy_valid" in checks
    ...
    assert "conformance_governance_contract_valid" in checks
    ...
    assert "decision_record_operating_principles_contract_valid" in checks
...
# Layer: contract
def test_runtime_truth_contract_drift_report_fails_when_decision_record_operating_principles_contract_invalid(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from orket.runtime import runtime_truth_drift_checker as checker
...
# Layer: contract
def test_runtime_truth_contract_drift_report_fails_when_conformance_governance_contract_invalid(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from orket.runtime import runtime_truth_drift_checker as checker
    ...
    payload = checker.runtime_truth_contract_drift_report()
    target = next(
        row for row in payload["checks"] if row["check"] == "conformance_governance_contract_valid"
    )
    assert target["ok"] is False
