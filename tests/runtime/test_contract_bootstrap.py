...
""".strip()
    ...
    assert set(snapshots.tool_contract_snapshot["tool_contracts"][0].keys()) == {
        "tool_name",
        "ring",
...
# Layer: contract
def test_load_runtime_contract_snapshots_rejects_missing_capability_profile(tmp_path: Path) -> None:
    _write(
        tmp_path / "core" / "artifacts" / "schema_registry.yaml",
        """
registry_version: "1.0"
...
""".strip()
    ...
    with pytest.raises(ValueError, match="requires capability_profile"):
