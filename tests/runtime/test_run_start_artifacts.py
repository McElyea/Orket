...
def test_capture_run_start_artifacts_writes_required_run_start_files(tmp_path: Path) -> None:
    ...
    assert payload["long_session_soak_test_contract"]["schema_version"] == "1.0"
    soak_check_ids = [row["check_id"] for row in payload["long_session_soak_test_contract"]["checks"]]
    assert "stub_provider_terminal_event_per_turn" in soak_check_ids
    ...
    assert "extension_runtime_models_unavailable_returns_truthful_degraded_failure" in degradation_check_ids
    ...
    assert "decision_log_template_sections_present" in decision_check_ids
    ...
    assert Path(payload["decision_record_operating_principles_contract_path"]).exists()
