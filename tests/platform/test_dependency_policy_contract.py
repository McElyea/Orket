...
def test_dependency_direction_and_snapshot_use_canonical_policy(tmp_path: Path) -> None:
    ...
    check = subprocess.run(
        ["python", "scripts/governance/check_dependency_direction.py", "--out", str(check_out)],
        capture_output=True,
        text=True,
    )
    ...
    snapshot = subprocess.run(
        [
            "python",
            "scripts/governance/export_dependency_graph.py",
            "--out-json",
...
def test_dependency_direction_can_fail_on_legacy_budget_overrun(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            "python",
            "scripts/governance/check_dependency_direction.py",
            "--out",
            str(check_out),
            "--legacy-edge-max",
            "0",
