...
async def graph_runtime(root, case, *, root_first=True):
    db = Path(await asyncio.to_thread(resolve_runtime_db_path))
    if case == "legacy_done":
...
async def test_graph_dependency_labels_require_current_retained_acceptance(tmp_path, monkeypatch, case):
    ...
    async with graph_runtime(tmp_path, case) as (app, _):
...
async def test_graph_rechecks_evidence_on_each_read_and_persists_observed_projection(tmp_path, monkeypatch, root_first):
    monkeypatch.setenv("ORKET_API_KEY", "test-key")
    async with graph_runtime(tmp_path, "done", root_first=root_first) as (app, engine):
...
async def test_graph_reports_dependency_cycle_without_changing_lifecycle(tmp_path, monkeypatch):
    monkeypatch.setenv("ORKET_API_KEY", "test-key")
    async with graph_runtime(tmp_path, "missing") as (app, engine):
...
async def test_graph_handoffs_remain_observations_and_snapshot_failure_is_logged(tmp_path, monkeypatch, caplog, root_first):
    monkeypatch.setenv("ORKET_API_KEY", "test-key")
    async with graph_runtime(tmp_path, "done", root_first=root_first) as (app, _):
