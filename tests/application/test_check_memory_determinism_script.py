...
def test_check_memory_determinism_fails_when_oversized_trace_has_no_truncation_marker(tmp_path: Path) -> None:
    trace = tmp_path / "memory_trace.json"
    trace.write_text(
        json.dumps(
            {
    ...
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert "trace:missing:truncation_marker" in payload["failures"]


def test_check_memory_determinism_allows_oversized_trace_with_truncation_marker(tmp_path: Path) -> None:
    trace = tmp_path / "memory_trace.json"
    trace.write_text(
        json.dumps(
            {
