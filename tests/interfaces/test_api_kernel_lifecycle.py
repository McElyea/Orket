...
def test_kernel_api_end_session_publishes_operator_cancel_for_unfinished_trace(monkeypatch) -> None:
    ...
    monkeypatch.setattr(api_module._get_engine(client.app), "control_plane_execution_repository", execution_repo)
    monkeypatch.setattr(api_module._get_engine(client.app), "control_plane_repository", record_repo)
    monkeypatch.setattr(api_module._get_engine(client.app), "control_plane_publication", publication)
    monkeypatch.setattr(
        api_module._get_engine(client.app),
        "kernel_action_control_plane",
        KernelActionControlPlaneService(
...
def test_kernel_replay_endpoint_routes_to_engine_and_propagates_failure_codes(monkeypatch) -> None:
    ...
    monkeypatch.setattr(api_module._get_engine(client.app), "kernel_replay_run", fake_kernel_replay_run)
